...
class _BudgetsScreenState extends ConsumerState<BudgetsScreen> {
  ...
  void initState() {
    super.initState();
    // FIXED: Use expenseCategories instead of categories
        ...
        children: [
          ...
          ...AppConstants.expenseCategories.map((category) =>
              _BudgetCard(
                ...
                currentSpent: _getCategorySpentAmount(category),
                budget: budgets[category],
                isOverBudget: _isCategoryOverBudget(category),
                isCloseToBudget: _isCategoryCloseToBudget(category),
  ...
  // FIXED: Helper methods to get budget data
  double _getCategorySpentAmount(String category) {
    final transactions = ref.read(transactionsProvider);
    final repository = ref.read(budgetRepositoryProvider);
    return repository.getCategorySpentAmount(category, DateTime.now(), transactions);
  }

  bool _isCategoryOverBudget(String category) {
    final transactions = ref.read(transactionsProvider);
    final repository = ref.read(budgetRepositoryProvider);
    return repository.isCategoryOverBudget(category, DateTime.now(), transactions);
  }

  bool _isCategoryCloseToBudget(String category) {
    final transactions = ref.read(transactionsProvider);
    final repository = ref.read(budgetRepositoryProvider);
    return repository.isCategoryCloseToBudget(category, DateTime.now(), transactions);
  }
  ...
  void _saveBudget(String category, double amount) {
    ...
    // FIXED: Use BudgetActions instead of direct notifier access
    BudgetActions.setBudget(ref, budget);
